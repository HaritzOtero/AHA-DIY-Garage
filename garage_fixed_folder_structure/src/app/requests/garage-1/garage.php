...
while($row <= 7){ //ROW
    ...
    while ($x_day <= $S_saturday){ //COLUMN
        ...
        if ($S_week < $T_week){ // DONT TOUCH
            ...
            echo "<td>";
            $buy = mysqli_query($link,"select buy from garage_1 where Hours = '$value' and week = '$S_week'");
            ...
            if(mysqli_num_rows($buy)!=0){
                ...
                if($buy == 1){
                    ...
                    $emaitza=mysqli_query($link,"select * from garage_1 where Hours = '$value' and session = '$session' and week = '$S_week' and buy = '0'");
			...
			<TABLE id ="tabla" >
				...
				<?php
					while($erregistroa = mysqli_fetch_array($emaitza))
					{
						...
						?>
						<div>
							<form action="../products/add-product-1.php" method="post">
				...
				<h2>Selected hours:</h2>
			 	<div>
					<?php 
						...
						$astebete = $egune * 7;
						$showHours = mysqli_query($link,"select Hours, week from garage_1 where session = '$session' and buy = '0' ORDER by week ASC");
